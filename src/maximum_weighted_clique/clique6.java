...
public class clique6 {
	...
	public static void main(String[] args) throws IOException{
		...
		new clique6(scanner, TIME_LIMIT, switch_number);
	}
	
	public clique6(Scanner sc, long limit, int switchNum) throws IOException{
	...
	void expand(int n, int[] vset, int[] upper, int[] degree) {
		...
		for(int i = 0; i < n; i++) {
			...
			boolean[] adjv = adj[v];
			for(int j = i+1; j < n; j++) {
				if((adjv[vset[j]])&(degree[v]*0.49 < degree[vset[j]])) {
					vset2[n2++] = vset[j];
				}
			}
			// v�� �����ϴ� ������ ���� ��� (���� �ؾߵ� �κ�)
