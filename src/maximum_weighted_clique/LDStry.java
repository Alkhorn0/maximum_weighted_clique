...
public class LDStry {
    ...
    List<Integer> discrepency = new ArrayList<Integer>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        Scanner scanner
            = new Scanner(new BufferedReader(new FileReader("C:\\Users\\김영재\\1000_0.7_6_10.txt")));
    ...
    public LDStry(Scanner sc, long limit, int switchNum) throws IOException {
        ...
        expand(Vnbr, vset, upper,current, degree, 0);
        //printRecord();
        //System.out.println('x');
        // 큐에서 부분문제를 빼내어 LDS의 D=1부터 탐색진행
        int d = 0;
        while(true){
        	// 큐가 비면 종료
        	...
        	d = discrepency.remove(0); // discrepancy 할당
        	...
        	if(vsettmp[0] != Vnbr && vsettmp[0] != -1) { // 부분문제가 비어있지 않으면, 
        		...
        		expand(vsetnum,vsettmp,upper,currenttmp, degree, d); // 부분 문제와 현재 형성중인 클리크를 입력
        	}
        }
        printRecord();
    }
     ...
     * @param upper ��E�̒l
     */
    public void expand(int n, int[] vset, int[] upper, int[] currentx, int[] degree, int d) {
       ...
       // 	printRecord();
        ...
        for(int i = 0; i < 1; ++i) {
            ...
            int n2 = 0;
            boolean[] adjv = adj[v];
            /*PriorityQueue<Integer> vset4 = new PriorityQueue<>(n-i ,new Comparator<Integer>() {
            	...
            	public int compare(Integer o1, Integer o2) {
            		...
            		/*if((wt[o1]*degree[o1] < wt[o2]*degree[o2])) {
            			return -1;
            		}
            ...
            d++;
            if(vset3.length != 0 && d < 6) { //부분 문제의 정점집합이 있는 경우 이하를 실행
            ...
            if(n2 == 0) { // 분기에서의 '잎'의 처리
                ...
                //}
                expand(n2, vset2, upper2, current, degree, d);
            }
            --currentSize;
            currentWeight -= wt[v];
        }
